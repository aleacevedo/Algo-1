...
class VentanaReproductor(pyglet.window.Window):
	...
	def __init__(self, reproductor):
		...
		self.lista_cola_reproduccion = WidgetColaReproduccion(self.reproductor.cola_de_reproduccion, 0, self.label_cola_reproduccion.y - self.ALTURA_LABELS * 2, self.batch)
...
class WidgetColaReproduccion():
	...
	def __init__(self, cola_de_reproduccion, x, y, batch):
		...
		self.cola=cola_de_reproduccion
		self.batch=batch
		...
		for i in range(10):
				if(i == 0):
					self.label_cancion.append(pyglet.text.Label(" ", y = y, anchor_y = "top", height = self.ALTURA_LABELS, batch = self.batch))
				else:
					self.label_cancion.append(pyglet.text.Label(" ", y = self.label_cancion[i-1].y - self.ALTURA_LABELS, anchor_y = "top", height = self.ALTURA_LABELS, batch = self.batch))
		self.actualizar()
